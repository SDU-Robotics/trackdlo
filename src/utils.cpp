...
bool isBetween(MatrixXd x, MatrixXd a, MatrixXd b)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    if (!(a(0, i) - 0.0001 <= x(0, i) && x(0, i) <= b(0, i) + 0.0001) && !(b(0, i) - 0.0001 <= x(0, i) && x(0, i) <= a(0, i) + 0.0001))
    {
      in_bound = false;
    }
  }
...
std::vector<MatrixXd> line_sphere_intersection(MatrixXd point_A, MatrixXd point_B, MatrixXd sphere_center, double radius)
{
  ...
  double b = 2 * ((point_B(0, 0) - point_A(0, 0)) * (point_A(0, 0) - sphere_center(0, 0)) + (point_B(0, 1) - point_A(0, 1)) * (point_A(0, 1) - sphere_center(0, 1)) +
